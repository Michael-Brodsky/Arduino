...
DigitalClock::DigitalClock(EEPROMStream& eeprom, Callback callback) :
	status_(TimeStatus::timeNotSet), mode_(), adjusted_time_(), 
	alarm_enabled_(), alarm_time_(), alarm_active_(), temp_time_(), 
	current_(fields_.cbegin()), callback_(callback), eeprom_(eeprom)
{
...
void DigitalClock::serialize(EEPROMStream& eeprom) const 
{
	eeprom << alarm_enabled_;
	eeprom << alarm_time_;
}

void DigitalClock::deserialize(EEPROMStream& eeprom)
{
	eeprom >> alarm_enabled_;
	eeprom >> alarm_time_;
}
...
//{
//	(void)EEPROMStream::update(EepromAlarmEnabled, alarm_enabled_);
//	(void)EEPROMStream::update(EepromAlarmTime, alarm_time_);
//}
